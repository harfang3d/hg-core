# Try to find OpenGLES2. Once done this will define:
#     OPENGLES_VERSION
#     OPENGLES2_FOUND
#     OPENGLES2_INCLUDE_DIRS
#     OPENGLES2_LIBRARIES
#     OPENGLES2_DEFINITIONS

find_package(PkgConfig)

find_package(EGL REQUIRED)

pkg_check_modules(PC_OPENGLES2 glesv2)

if (PC_OPENGLES2_FOUND)
    set(OPENGLES2_DEFINITIONS ${PC_OPENGLES2_CFLAGS_OTHER})
endif ()

find_path(OPENGLES2_INCLUDE_DIRS NAMES GLES2/gl2.h
    HINTS ${PC_OPENGLES2_INCLUDEDIR} ${PC_OPENGLES2_INCLUDE_DIRS}
)

set(OPENGLES2_NAMES ${OPENGLES2_NAMES} glesv2 GLESv2)
find_library(OPENGLES2_LIBRARY NAMES ${OPENGLES2_NAMES}
    HINTS ${PC_OPENGLES2_LIBDIR} ${PC_OPENGLES2_LIBRARY_DIRS}
)

set(OPENGLES_VERSION 2)

if(IS_ABSOLUTE "${OPENGLES2_LIBRARY}")
    add_library(OpenGL::GLES2 UNKNOWN IMPORTED)
    set_target_properties(OpenGL::GLES2 PROPERTIES
                        IMPORTED_LOCATION "${OPENGLES2_LIBRARY}")
else()
    add_library(OpenGL::GLES2 INTERFACE IMPORTED)
    set_target_properties(OpenGL::GLES2 PROPERTIES
                        IMPORTED_LIBNAME "${OPENGLES2_LIBRARY}")
endif()
set_target_properties(OpenGL::GLES2 PROPERTIES
    INTERFACE_INCLUDE_DIRECTORIES "${OPENGLES2_INCLUDE_DIRS}")

include(FindPackageHandleStandardArgs)
find_package_handle_standard_args(OpenGLES2 REQUIRED_VARS OPENGLES2_INCLUDE_DIRS OPENGLES2_LIBRARY
                                  FOUND_VAR OPENGLES2_FOUND)

mark_as_advanced(OPENGLES_VERSION OPENGLES2_INCLUDE_DIRS OPENGLES2_LIBRARY)
