...
#include <limits>
#include <memory>
#include <srombauts/shared_ptr.hpp>
#include <vector>
...
struct BoundToNodeInstanceAnim {
	int kf;
	shared_ptr<SceneBoundAnim> bound_anim; // circular definition mandates a heap allocation
};
...
struct SceneAnim {
	SceneAnim() : t_start(0), t_end(0), frame_duration(time_from_ms(1000 / 20)), flags(0) {} // default to 20 fps
	...
	time_ns frame_duration;
	uint8_t flags;
};

typedef gen_ref SceneAnimRef;
...
//
typedef gen_ref ScenePlayAnimRef;
extern const ScenePlayAnimRef InvalidScenePlayAnimRef;

//
enum ProbeType { PT_Sphere, PT_Cube, PT_Count };

struct Probe {
	Probe() : type(PT_Sphere), parallax(0) {}
...
struct LoadSceneContext {
	LoadSceneContext() : recursion_level(0) {}
...
public:
	...
	*/
	...
	Transform CreateTransform(const Vec3 &pos, const Vec3 &rot = Vec3(0, 0, 0), const Vec3 &scl = Vec3(1, 1, 1), NodeRef parent = InvalidNodeRef);
	Transform CreateTransform(const Mat4 &mtx, NodeRef parent = InvalidNodeRef);
	...
	Object CreateObject(const ModelRef &model, const std::vector<Material> &materials = std::vector<Material>());
	...
	Light CreatePointLight(float radius, const Color &diffuse = Color(1, 1, 1), float diffuse_intensity = 1, const Color &specular = Color(1, 1, 1),
	...
	Light CreateSpotLight(float radius, float inner_angle, float outer_angle, const Color &diffuse = Color(1, 1, 1), float diffuse_intensity = 1,
		const Color &specular = Color(1, 1, 1), float specular_intensity = 1, float priority = 0, LightShadowType shadow_type = LST_None,
		float shadow_bias = default_shadow_bias);
	/// Create a Node with a Transform and a linear Light component.
	Light CreateLinearLight(const Color &diffuse = Color(1, 1, 1), float diffuse_intensity = 1, const Color &specular = Color(1, 1, 1),
		float specular_intensity = 1, float priority = 0, LightShadowType shadow_type = LST_None, float shadow_bias = default_shadow_bias,
	...
	struct Canvas {
		Canvas() : clear_z(true), clear_color(true), color(0, 0, 0, 1) {}
		bool clear_z, clear_color;
		Color color;
	};
	...
	struct Environment {
		Environment() : ambient(0, 0, 0, 1), fog_color(0, 0, 0, 1), fog_near(0), fog_far(0) {}
	...
	*/
	...
	SceneAnimRef GetSceneAnim(const std::string &name) const;
	...
	ScenePlayAnimRef PlayAnim(SceneAnimRef ref, AnimLoopMode loop_mode = ALM_Once, Easing easing = E_Linear, time_ns t_start = UnspecifiedAnimTime,
		time_ns t_end = UnspecifiedAnimTime, bool paused = false, float t_scale = 1);
	...
	bool Load_binary(const Reader &ir, const Handle &h, const std::string &name, const Reader &deps_ir, const ReadProvider &deps_ip,
		PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx, uint32_t flags = LSSF_All);
	...
	bool LoadNodes_binary(const Reader &ir, const Handle &h, const std::string &name, const Reader &deps_ir, const ReadProvider &deps_ip,
		PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx);

	//
	void Save_json(rapidjson::Document &jd, rapidjson::Value &js, const PipelineResources &resources, uint32_t flags = LSSF_All,
		const std::vector<NodeRef> *nodes_to_save = nullptr) const;
	bool Load_json(const rapidjson::Value &js, const std::string &name, const Reader &deps_ir, const ReadProvider &deps_ip, PipelineResources &resources,
...
private:
	...
	template <int I> inline ComponentRef GetNodeComponentRef_(NodeRef ref) const {
		const Node_ *node_ = this->GetNode_(ref);
		return node_ ? node_->components[I] : invalid_gen_ref;
	}

	template <int I> inline void SetNodeComponentRef_(NodeRef nref, ComponentRef cref) {
		if (Node_ *node_ = this->GetNode_(nref))
...
public:
	...
	struct Light_ {
		Light_()
			: type(LT_Point), shadow_type(LST_None), diffuse(1, 1, 1, 1), diffuse_intensity(1), specular(1, 1, 1, 1), specular_intensity(1), radius(0),
			  inner_angle(Deg(30.f)), outer_angle(Deg(45.f)), pssm_split(10.f, 50.f, 100.f, 200.f), priority(0.f), shadow_bias(default_shadow_bias) {}

		LightType type;
		LightShadowType shadow_type;
	...
	struct RigidBody_ { // 6B
		RigidBody_()
			: type(RBT_Dynamic), linear_damping(pack_float<uint8_t>(0.f)), angular_damping(pack_float<uint8_t>(0.f)), restitution(pack_float<uint8_t>(0.f)),
			  friction(pack_float<uint8_t>(0.5f)), rolling_friction(pack_float<uint8_t>(0.f)) {}

		RigidBodyType type;

		uint8_t linear_damping;
		uint8_t angular_damping;

		uint8_t restitution;
		uint8_t friction;
		uint8_t rolling_friction;
	};
	...
	//
	struct Collision_ {
		Collision_() : type(CT_Cube), mass(Kg(1.f)) {}
	...
	std::set<ComponentRef> scripts_detached_from_scene; // will be used to call OnDetachFromScene
	std::map<ComponentRef, std::set<NodeRef> > scripts_detached_from_nodes; // will be used to call OnDetachFromNode

	//
	struct Instance_ {
		Instance_() : loop_mode(ALM_Once) {}
	...
	//
	friend void LoadComponent(Transform_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(Camera_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(Object_ *data_, const rapidjson::Value &js, const Reader &deps_ir, const ReadProvider &deps_ip, PipelineResources &resources,
		const PipelineInfo &pipeline, bool queue_model_loads, bool queue_texture_loads, bool do_not_load_resources);
	friend void LoadComponent(Light_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(RigidBody_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(Collision_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(Instance_ *data_, const rapidjson::Value &js);
	friend void LoadComponent(Script_ *data_, const rapidjson::Value &js);

	friend void SaveComponent(const Scene::Transform_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const Scene::Camera_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const Scene::Object_ *data_, rapidjson::Document &jd, rapidjson::Value &js, const PipelineResources &resources);
	friend void SaveComponent(const Light_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const RigidBody_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const Collision_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const Instance_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	friend void SaveComponent(const Script_ *data_, rapidjson::Document &jd, rapidjson::Value &js);
	...
	//
	static const uint8_t SPAF_Paused = 0x1;
...
//
struct SceneRef {
	SceneRef(Scene *scene_ = nullptr) : scene(scene_), ref_count(0) {}
	Scene *scene;
	uint32_t ref_count;
};

/// Helper function to create a Node with a Transform component then parent all root nodes in the scene to it.
Node CreateSceneRootNode(Scene &scene, std::string name = std::string(), const Mat4 &mtx = Mat4::Identity);
...
Node CreatePointLight(Scene &scene, const Mat4 &mtx, float radius, const Color &diffuse = Color(1, 1, 1), float diffuse_intensity = 1,
	const Color &specular = Color(1, 1, 1), float specular_intensity = 1, float priority = 0.f, LightShadowType shadow_type = LST_None,
	float shadow_bias = default_shadow_bias);
Node CreateSpotLight(Scene &scene, const Mat4 &mtx, float radius, float inner_angle, float outer_angle, const Color &diffuse = Color(1, 1, 1),
	float diffuse_intensity = 1, const Color &specular = Color(1, 1, 1), float specular_intensity = 1, float priority = 0,
	LightShadowType shadow_type = LST_None, float shadow_bias = default_shadow_bias);
Node CreateLinearLight(Scene &scene, const Mat4 &mtx, const Color &diffuse = Color(1, 1, 1), float diffuse_intensity = 1,
	const Color &specular = Color(1, 1, 1), float specular_intensity = 1, float priority = 0, LightShadowType shadow_type = LST_None,
	float shadow_bias = default_shadow_bias, const Vec4 &pssm_split = default_pssm_split);
...
bool SaveSceneJsonToFile(const std::string &path, const Scene &scene, const PipelineResources &resources, uint32_t save_flags = LSSF_All);
bool SaveSceneJsonToData(Data &data, const Scene &scene, const PipelineResources &resources, uint32_t save_flags = LSSF_All);
bool SaveSceneBinaryToFile(const std::string &path, const Scene &scene, const PipelineResources &resources, uint32_t save_flags = LSSF_All, bool debug = false);
...
bool IsBinarySceneFile(const std::string &path);
bool IsBinarySceneAsset(const std::string &name);

bool LoadSceneJsonFromFile(
	const std::string &path, Scene &scene, PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx, uint32_t flags = LSSF_All);
bool LoadSceneJsonFromAssets(
	const std::string &name, Scene &scene, PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx, uint32_t flags = LSSF_All);
...
bool LoadSceneBinaryFromFile(const std::string &path, Scene &scene, PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx,
	uint32_t flags = LSSF_All, bool debug = false);
bool LoadSceneBinaryFromAssets(const std::string &name, Scene &scene, PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx,
	uint32_t flags = LSSF_All, bool debug = false);
bool LoadSceneBinaryFromData(const Data &data, const std::string &name, Scene &scene, const Reader &deps_ir, const ReadProvider &deps_ip,
	PipelineResources &resources, const PipelineInfo &pipeline, LoadSceneContext &ctx, uint32_t flags = LSSF_All, bool debug = false);
