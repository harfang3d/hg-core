...
void test_file_rw_interface() {
	...
	{ 
		...
		{
			ScopedWriteHandle h(g_file_write_provider, filename);
		...
		TEST_CHECK(g_file_read_provider.is_file(filename) == true);
		TEST_CHECK(g_file_read_provider.is_file("/tmp/") == false);

		{ 
			ScopedReadHandle h(g_file_read_provider, filename);
