...
std::string slice(const std::string &str, ptrdiff_t from, ptrdiff_t count) {
	...
	if (count > (static_cast<ptrdiff_t>(l) - from)) {
...
std::string reduce(const std::string &str, const std::string &fill, const std::string &pattern) {
	// trim first
	std::string res = strip(str, pattern);

	// replace sub ranges
	size_t begin_space = res.find_first_of(pattern);

	while (begin_space != std::string::npos) {
		const size_t endSpace = res.find_first_not_of(pattern, begin_space);
		const size_t range = endSpace - begin_space;

		res.replace(begin_space, range, fill);

		const size_t newStart = begin_space + fill.length();
		begin_space = res.find_first_of(pattern, newStart);
	}
...
std::string lstrip(const std::string &str, const std::string &pattern) {
	...
	if (str_begin != std::string::npos) {
		res = str.substr(str_begin, str.length() - str_begin);
	}
...
std::string strip(const std::string &str, const std::string &pattern) {
	return lstrip(rstrip(str, pattern), pattern);
}
...
std::string strip_space(const std::string &str) {
	return lstrip_space(rstrip_space(str));
}

//
std::string strip_prefix(const std::string &str, const std::string &prefix) {
	return starts_with(str, prefix) ? slice(str, prefix.length()) : str;
}

std::string strip_suffix(const std::string &str, const std::string &suffix) {
	return ends_with(str, suffix) ? slice(str, 0, -static_cast<int>(suffix.length())) : str;
}
...
std::string utf32_to_utf8(const std::u32string &str) {
	...
	return std::string(reinterpret_cast<char *>(utf8string.data()), utf8string.size());
}
...
//
static inline char _tolower_impl(char c) {
...
static inline char _toupper_impl(char c) {
...
std::string word_wrap(const std::string &str, int width, int lead, char lead_char) {
	...
	std::string res;
	res.reserve(str.length() + (str.length() / width) * (lead + 1));
...
std::string pad_left(const std::string &str, int padded_width, char padding_char) {
	...
	const int count = padded_width - static_cast<int>(str.size());

	if (count > 0) {
		res = std::string(count, padding_char) + str;
	} else {
		res = str;
	}
...
std::string pad_right(const std::string &str, int padded_width, char padding_char) {
	...
	if (count > 0) {
		res = str + std::string(count, padding_char);
	} else {
		res = str;
	}
