...
inline float GetArea(const MinMax &mm) {
	return (mm.mx.x - mm.mn.x) * (mm.mx.y - mm.mn.y) * (mm.mx.z - mm.mn.z);
}

/// Return the minmax center.
inline Vec3 GetCenter(const MinMax &mm) {
	return (mm.mn + mm.mx) * 0.5F;
}

/// Return the minmax size.
inline Vec3 GetSize(const MinMax &mm) {
	return mm.mx - mm.mn;
}
...
inline bool Overlap(const MinMax &a, const MinMax &b, Axis axis) {
	return !(b.mn[axis] > a.mx[axis] || b.mx[axis] < a.mn[axis]);
}
...
inline bool operator==(const MinMax &a, const MinMax &b) {
...
inline MinMax Union(const MinMax &a, const MinMax &b) {
	return MinMax(Min(a.mn, b.mn), Max(a.mx, b.mx));
}

/// Return the union of a minmax and vector.
inline MinMax Union(const MinMax &mm, const Vec3 &p) {
	return MinMax(Min(mm.mn, p), Max(mm.mx, p));
}
...
inline MinMax MinMaxFromPositionSize(const Vec3 &p, const Vec3 &s) {
	return MinMax(p - s * 0.5F, p + s * 0.5F);
}
